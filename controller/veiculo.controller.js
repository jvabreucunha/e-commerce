...
const buscarPorModelo = async (req, res) => {
  try {
    ...
    if (!modelo) {
      return res
        .status(400)
        .json({ message: "O parâmetro 'modelo' é obrigatório." });
    }
...
const buscarMaisBuscados = async (req, res) => {
  try {
    const ranking = await redis.zRangeWithScores("veiculos:ranking", 0, 9, {
      REV: true,
    });
...
const atualizar = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Veículo atualizado com sucesso!", veiculo });
  } catch (err) {
...
const deletar = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ message: "Você não tem permissão para deletar este veículo" });
    }
